...
SLOT=2
PN1=${PN/+/}${SLOT}-x11
PN2=${PN/+/}${SLOT}-x11-devel
PN3=${PN/+/}${SLOT}-x11-doc
PN4=${PN/+/}${SLOT}-x11-misc
P=${PN}-${PV}
P1=${PN1}-${PV}
P2=${PN2}-${PV}
P3=${PN3}-${PV}
...
cygmake() {
	cd ${B}
	make ${MAKEOPTS} CFLAGS="-I${S}" || exit 1
}
...
cygdoc() {
	...
	cp -r ${S}/docs/reference ${D}/usr/share/doc/${P1}/html

	for f in ${install_docs} ; do
		if [ -f ${S}/${f} ] ; then
			/bin/install -m 644 ${S}/${f} ${D}/usr/share/doc/${P1} ;
			# gzip -q ${D}/usr/share/doc/${P1}/${f} ;
		fi
	done
...
GCONF_CONFIG_SOURCE="xml::/etc/gconf/gconf.xml.defaults" \
	...
	/etc/gconf/schemas/*.schemas > /dev/null 2>&1
_EOF
	fi
...
cygpkg() {
	...
	if [ "${MULTIPKG}" -eq 1 ] ; then
		...
		tar cvjf ${bin_pkg2} usr/bin/*-config \
			usr/include/ \
			usr/lib/ \
			usr/share/aclocal
	else \
